...
<div>
   {{-- Card --}}
    <div class="card">
        ...
        <x-card-header title="{{ __('Registros de contratos') }}" 
            description="{{ __('Contratos con acceso a') }} {{ $group?->business_name ?? $groupCode }}"
            textColor="text-plus" icon="ti tabler-users" iconColor="bg-label-info">
            @can('hr.contratos.create')
            <button onclick="Livewire.dispatch('openCreateModal')"
                ...
                title="{{ __('Nuevo Contrato') }}">
                <i class="ti tabler-plus me-md-1"></i>
                <span class="d-none d-md-inline ms-1">{{ __('Nuevo Contrato') }}</span>
            </button>
            @endcan
        </x-card-header>

        {{-- Componente Livewire --}}
        <livewire:hr-contrato-manager />
    </div>
</div>
@endsection
