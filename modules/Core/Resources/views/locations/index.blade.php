...
<div>
   {{-- Card --}}
    <div class="card">
      ...
      <x-card-header title="{{ __('Gestión de Locales') }}" 
         description="{{ __('Locales con acceso a') }} {{ $group?->business_name ?? $groupCode }}"
         ...
         <button onclick="Livewire.dispatch('openCreateModal')"
               ...
               title="{{ __('Nuevo Locales') }}">
               <i class="ti tabler-plus me-md-1"></i>
               <span class="d-none d-md-inline ms-1">{{ __('Nuevo Locales') }}</span>
         </button>
         @endcan
      ...
      {{-- Componente Livewire --}}
      <livewire:core-location-manager />
    </div>
</div>
@endsection
